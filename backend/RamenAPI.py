...
# get setlist endpoint
@app.route('/get/filter/<brand>/<variety>/<style>/<country>/<stars>', methods=["GET"])
def get_filter(brand, variety, style, country, stars):
    # from koble database's setlist collection, find document that has concert_id that matches concert_id from
    # the request's URL parameter
    # also do not return _id from document, or it will break endpoint
    setlist = mycol.find({"Brand": brand, "Variety": variety, "Style": style, "Country": country, "Stars": stars}, {"_id": False})
    return list(setlist)
